...
def predict():
    try:
        ...
        xgBoostResult = predictXGBoost(data)
        lrResult = predictXGBoost(data)

        if xgBoostResult["diabetes_prediction"] == lrResult["diabetes_prediction"]:
...
def predictLR(data):
    ...
    # 2. Попередня обробка даних пацієнта
    label_encoders = {
    ...
    scaler = load(os.path.join(Config.MODELS_DIR, 'diabetes_logisticRegression_scaler.joblib'))
...
def predictXGBoost(data):
    # 1. Завантаження моделі
    model = load(os.path.join(Config.MODELS_DIR, 'diabetes_xgboost_model.joblib'))
    ...
    # Завантаження скейлера
    scaler = load(os.path.join(Config.MODELS_DIR, 'diabetes_xgboost_scaler.joblib'))
    ...
    # Формування відповіді
    result = {
        "diabetes_prediction_model": "XGBoost",
        "diabetes_prediction": int(diabetes_prediction[0]),  # 0 або 1
        "diabetes_probability": round(float(diabetes_probability[0]), 2),
        "msg": ""
    }
